...
`validationSet` defaults to `trainingSet`. That is the right default for a first run and the wrong number to report: the search selected against those instances, so `bestScore` is fitted to them. [Held-out evaluation](./evaluation.md#held-out-evaluation) is how to find out what that cost.
...
- **Caching.** Cache keys include the split, complete candidate, and instance ID. Cache hits do not count against the metric budget. Instance IDs default to a content hash, falling back to the row position for a datum the hash cannot read — one that will not serialize, and one whose content JSON does not reach, such as a Map, a Set, or a class instance holding its state privately. Provide `instanceId` for non-serializable data or readable trace IDs. Set `cache: false` to disable caching.
