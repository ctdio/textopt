...
describe("createEpochShuffledSampler", () => {
  test("returns exactly minibatchSize indices", () => {
    ...
    const trainingSet = [1, 2, 3, 4, 5, 6];

    expect(
      sample({ trainingSet, iteration: 0, rng: createSeededRng(1) }),
    ).toHaveLength(3);
  });

  test("covers the whole trainingSet over one epoch", () => {
    const sample = createEpochShuffledSampler({ minibatchSize: 3 });
    const trainingSet = [1, 2, 3, 4, 5, 6];
    const rng = createSeededRng(1);

    const seen = new Set<number>([
      ...sample({ trainingSet, iteration: 0, rng }),
      ...sample({ trainingSet, iteration: 1, rng }),
    ]);

    expect(seen).toEqual(new Set([0, 1, 2, 3, 4, 5]));
  });

  test("pads a trainingSet smaller than the minibatch", () => {
  ...
  test("throws on an empty trainingSet", () => {
