...
- **`retry`** re-runs instances the adapter marked `transient`. Nothing is marked for you: `isTransient` defaults to classifying no error at all, so out of the box a failed rollout scores zero against the candidate that hit it rather than being retried. A run that finished that way says how many times in `warnings`. A rate limit or a 5xx otherwise costs the instance either an unexplained zero or a hole in the candidate's coverage. Retries are charged like any other rollout but never overdraw the budget. Defaults to two attempts, 500 ms apart, doubling.
