...
- **`retry`** covers two things. It re-runs instances the adapter marked `transient` — and nothing is marked for you: `isTransient` defaults to classifying no error at all, so out of the box a failed rollout scores zero against the candidate that hit it rather than being retried. A run that finished that way says how many times in `warnings`. A rate limit or a 5xx otherwise costs the instance either an unexplained zero or a hole in the candidate's coverage. Rollout retries are charged like any other rollout but never overdraw the budget. It also re-runs a throwing `reflect` call, with nothing classified: a proposal model is one call standing between a run and its next candidate, and a run that ends there throws away rollouts it has already paid for. Nothing bounds what those attempts cost except `attempts` and whatever reflection ceiling the optimizer keeps, and each attempt counts against that ceiling. Defaults to two attempts, 500 ms apart, doubling.
...
What does change is what the provider sees. The limits multiply — `compare` concurrency, then the optimizer's, then the adapter's — so GEPA at `proposals: { concurrency: 4 }` over an adapter at `concurrency: 4` is sixteen calls in flight. Past what a provider tolerates this stops being a timing question: a throttled rollout comes back as a failure, which is never cached whatever else it is. An adapter whose `isTransient` recognises the throttle also leaves it out of the candidate's mean rather than scoring it a zero, and retries it at the run's expense; one that classifies nothing scores it zero and the run reports how often that happened. A run that spends its allowance on retries and measures its candidates on fewer instances than it asked for looks, from the outside, like a seed that went badly.
...
- **Caching.** Cache keys include the split, complete candidate, and instance ID. Cache hits do not count against the metric budget. Instance IDs default to a content hash, falling back to the row position for a datum whose whole content the hash cannot read — one that will not serialize, and one that JSON does not reach at all, such as a bare Map, a Set, or a class instance holding its state privately. The fallback reads the whole datum, not its fields: two rows that differ only inside a nested Map, Set, or function-valued property serialize alike, hash alike, and share cached scores. Pass `instanceId` for data like that, and for non-serializable data or readable trace IDs. Set `cache: false` to disable caching. Two kinds of score are never written whatever the key: a `transient` one, and a `failed` one — the stand-in an adapter returns after catching an error. Caching either would pin an instance to a number nothing measured, and a repeat run over a warm cache would reproduce it rather than the score.
